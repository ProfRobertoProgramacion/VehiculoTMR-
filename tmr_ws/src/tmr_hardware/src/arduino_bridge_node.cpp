...
template <typename T> static inline T clamp_val(T val, T min_val, T max_val) {
...
public:
  ArduinoBridgeNode() : Node("arduino_bridge_node"), serial_fd_(-1) {
    ...
    this->declare_parameter<int>("esc_forward_min_us",
                                 1540); // Umbral mínimo de movimiento
    this->declare_parameter<int>("esc_forward_max_us",
                                 1650); // Velocidad máxima segura
    ...
    RCLCPP_INFO(this->get_logger(),
                "Arduino Bridge iniciado en %s a %d baudios", port_.c_str(),
                baud_rate_);
  }
...
private:
  void initSerial() {
    ...
    if (serial_fd_ < 0) {
      // Auto-detección: si el puerto configurado no abre, probar alternativas
      // comunes
      ...
      for (const auto &cand : candidates) {
        if (cand != port_ && access(cand.c_str(), F_OK) == 0) {
          RCLCPP_INFO(
              this->get_logger(),
              "Puerto %s no disponible. Auto-detectado y probando %s...",
              port_.c_str(), cand.c_str());
          port_ = cand;
          serial_fd_ = open(port_.c_str(), O_RDWR | O_NOCTTY | O_NDELAY);
          if (serial_fd_ >= 0) {
            RCLCPP_INFO(this->get_logger(),
                        "¡Conectado exitosamente al puerto alternativo %s!",
                        port_.c_str());
            break;
          }
        }
    ...
    if (serial_fd_ < 0) {
      RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                           "No se puede abrir el puerto %s. Reintentando...",
                           port_.c_str());
      return;
    }
    ...
    if (tcgetattr(serial_fd_, &tty) != 0) {
      RCLCPP_ERROR(this->get_logger(),
                   "Error obteniendo atributos termios para %s", port_.c_str());
      close(serial_fd_);
      serial_fd_ = -1;
      return;
    }
    ...
    RCLCPP_INFO(this->get_logger(),
                "Puerto serie %s conectado y configurado exitosamente.",
                port_.c_str());
  }
  ...
  void cmdCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
    ...
    double steer_normalized =
        clamp_val(steer_rad / max_steering_angle_rad_, -1.0, 1.0);
    ...
    int target_servo = servo_center_deg_ -
                       static_cast<int>(steer_normalized * servo_range_deg_);
    ...
    if (v > 0.01) {
      ...
      target_esc = esc_forward_min_us_ +
                   static_cast<int>(
                       v_ratio * (esc_forward_max_us_ - esc_forward_min_us_));
    } else if (v < -0.01) {
  ...
  void heartbeat() {
    ...
    // Watchdog local: Si no se han recibido comandos de ROS en 500ms, enviar
    // orden de detención
    ...
    if (len > 0) {
      ...
      if (bytes_written < 0) {
        RCLCPP_WARN(this->get_logger(),
                    "Fallo escribiendo al puerto serie. Reiniciando conexión.");
        close(serial_fd_);
        serial_fd_ = -1;
      }
    }
